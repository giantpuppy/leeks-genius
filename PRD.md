...
> **上次更新**: 2026/06/15 — 月历首页反馈优化：分割线上提至第六排下方并进一步调紧余量；周视图左右滑动切换周目而非月份
...
| 日期 | 改动内容 | 关联文件/模块 |
|------|---------|-------------|
| 2026/06/15 | 月历首页反馈优化：月历容器高度精确为 6 行 + 分割线/手柄区余量；分割线上提至第六排下方并进一步调紧余量，月历底部叠加渐变遮罩、分割线下方增加向下渐变过渡，形成柔和预览感；`_onHeaderHorizontalSwipe` 区分月/周视图，月视图切换月份、周视图切换周目；新增 `_changeWeek` 处理跨月事件加载与选中日期跟随 | `calendar_screen.dart` |
